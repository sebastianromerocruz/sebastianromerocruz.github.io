...
d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson", (data) => {
    ...
    svg.append("text")
        .attr("class", "map-text")
        .attr("x", (d) => {
            return projection(mexicoCityCoords)[0] - 20;
        })
        .attr("y", (d) => {
            return projection(mexicoCityCoords)[1] - 10;
        })
        .attr("fill", "#ffc2c2")
        .text("MEX");
    ...
    svg.append("text")
        .attr("class", "map-text")
        .attr("x", (d) => {
            return projection(whitePlainsCoords)[0] - 20;
        })
        .attr("y", (d) => {
            return projection(whitePlainsCoords)[1] - 10;
        })
        .attr("fill", "#ffc2c2")
        .text("JFK");
    ...
    svg.append("text")
        ...
        .attr("y", 15)
        .attr("id", "hello")
        .text("Thanks D3.js for letting me make cute maps like these.")

    // TODO: ADD A COOL THING WHEN PLANE ARRIVES TO NYC
    console.log(projection(svg.select("#hello").attr("x")));
})
...
function addMapHoverListener(plane, route, routeBack) {
    $("#moved").hover(() => {
        ...
        transition(plane, routeBack, 1);
    });
}

// Plane animations
function transition(plane, route, speed) {
